...
# interrupting a blocked lock doesn't destroy the wait order
    ...
    [ # Run in a separate subshell with job control enabled.
      ...
      'bgshell2=$! ; ' .
      # (3.5) At 0.2s, wait for read lock; print first character then Z
      '(sleep 0.3 && ./osprdaccess -r 1 -l | sed s/$/Z/ && sleep 2) & ' .
      'bgshell3=$! ; ' .
      # (3.6) At 0.2s, wait for read lock; print first character then W
