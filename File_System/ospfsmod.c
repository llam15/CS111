...
{
	/* EXERCISE: Your code here */
  uint32_t *bitmap_vector = ospfs_block(OSPFS_FREEMAP_BLK);
  int i = 0;

  // Go through all blocks, find first free block
  for (i; i < ospfs_super->os_nblocks; i++) {
    // bit = 1 if free. 
    if (bitvector_test(bitmap_vector, i)) {
      // Clear (set to 0 to mark non-free) and return
      bitvector_clear(bitmap_vector, i);
      return i;
    }
  }
...
free_block(uint32_t blockno)
{
  ...
  // Set bit to 1 to indicate free
  bitvector_set(bitmap_vector, blockno);
}
...
indir_index(uint32_t b)
{
  ...
  // If b is inside indirect block
  else if (b < (OSPFS_NDIRECT + OSPFS_NINDIRECT))
    return 0;

  // If b is inside double indirect block
  else
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		// Find offset in current block
		uint32_t block_offset = *f_pos % OSPFS_BLKSIZE;

		// n = How much data left in block
		n = OSPFS_BLKSIZE - block_offset;
		
		// If amount to read is less than left in block,
		// only read amount needed
		if ((count - amount) < n)
		  n = count - amount;

		// Attempt to read
		retval = copy_to_user(buffer, data + block_offset, n);

		// If copy_to_user returns non-zero, there is an error
		if (retval != 0) { 
		  retval = -EFAULT;
		  goto done;
		}
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	// If appending to file, move file position to end of file
	...
	// Change file size if writing past end of file
	if (*f_pos + count > oi->oi_size) {
	  // If could not change size, return 0 (no characters written)
	...
	while (amount < count && retval >= 0) {
		...
		// If amount to write is less than left in block, 
		// only write amount needed?
		if ((count - amount) < n)
		  n = count - amount;
		
		// Attempt to write
		retval = copy_from_user(data + block_offset, buffer, n);

		// If copy from user returns non-zero, there is an error
		if (retval != 0) {
		  retval = -EFAULT;
		  goto done;
		}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  ospfs_direntry_t *blank_entry;
  int r = 0;
  uint32_t old_size = dir_oi->oi_size;

  // Go through all directory entries, return empty entry if found
  uint32_t i = 0;
  for(i; i < dir_oi->oi_size; i += OSPFS_DIRENTRY_SIZE) {
    blank_entry = ospfs_inode_data(dir_oi, i);
    if (blank_entry->od_ino == 0)
      return blank_entry;
  }

  // Increase size by one block
  r = change_size(dir_oi, dir_oi->oi_size + OSPFS_DIRENTRY_SIZE);
  if (r < 0)
    return ERR_PTR(r);

  // Return first empty entry in block
  blank_entry = ospfs_inode_data(dir_oi, old_size);
 
  // blank_entry should be zero'd out in add_block call in change_size
  return blank_entry;
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	for (entry_ino; entry_ino < ospfs_super->os_ninodes; entry_ino++) {
	  empty_inode = ospfs_inode(entry_ino);
	  if (empty_inode->oi_nlink == 0)
	    break;
	}

	// If no free inode, return no space
	...
	empty_inode->oi_ftype = OSPFS_FTYPE_DIR;
	empty_inode->oi_nlink = 1;
	empty_inode->oi_mode = mode;
	memset(empty_inode->oi_direct, 0, sizeof(uint32_t)*OSPFS_NDIRECT);
	empty_inode->oi_indirect = 0;
	empty_inode->oi_indirect2 = 0;
