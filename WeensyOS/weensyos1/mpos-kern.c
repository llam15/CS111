...
start(void)
{
	...
	for (i = 0; i < NPROCS; i++) {
		...
		proc_array[i].p_wait = 0; // No processes waiting
	}
...
static pid_t do_thread(process_t *parent);
static pid_t do_kill(process_t *parent);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_THREAD:
	       current->p_registers.reg_eax = do_thread(current);
	       run(current);

	case INT_SYS_KILL:
	       current->p_registers.reg_eax = do_kill(current);
	       run(current);
	...
	case INT_SYS_EXIT:
		...
		if (current->p_wait != 0) {
		  // Wake waiting process, set eax to current exit status, kill current process
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			proc_array[p].p_state == P_EMPTY; // Kill the zombie
		}
		else {
		  current->p_state = P_BLOCKED; // Set state to be blocked
		  proc_array[p].p_wait = current->p_pid; // Set p_wait of child process to current process pid
		}
		schedule();
	}
...
do_thread(process_t *parent)
{
  ...
  // If child is NULL, no empty processes available, return -1
  if (child == NULL)
    return -1;

  // Initialize the child to be runnable and exit status to 0
  // Set child's eip to start function
  // Set the eax register in the child to zero, so thread returns 0 to the child
 ...
 * do_kill
 *
 * Kills process. Returns pid of process killed
 * 
 **********************************************/

static pid_t
do_kill(process_t *caller)
{
  pid_t p = caller->p_registers.reg_eax;
  int retval;
  // Process to kill does not exist or is self. Return -1
  if (p <=0 || p >= NPROCS || 
       proc_array[p].p_state == P_EMPTY) {
    retval = -1;
    schedule();
    ...
    target->p_state = P_ZOMBIE; // Set to Zombie
    target->p_exit_status = -1; // Killed, so set exit status to -1

    if (target->p_wait != 0) {
      // Wake waiting process, set eax to current exit status, kill current process
      proc_array[target->p_wait].p_state = P_RUNNABLE; 
      proc_array[target->p_wait].p_registers.reg_eax = target->p_exit_status; 
      target->p_state = P_EMPTY; 
      run(&proc_array[target->p_wait]);
      retval = target->p_pid;
    }
    schedule();
  }
...
do_fork(process_t *parent)
{
  ...
  // Initialize the child to be runnable and exit status to 0
  // Copy the parent registers and the parent stack
  // Set the eax register in the child to zero, so fork returns 0 to the child
  child->p_state = P_RUNNABLE;
  child->p_registers = parent->p_registers;
  child->p_exit_status = 0;
